...
<template>
        ...
        <n-tab-pane name="notification" tab="Уведомления">
          <n-card title="Уведомления">
            <n-space vertical>
              ...
              <n-space>
                <n-button @click="createTestNotifications" type="primary">
  ...
  const createTestNotifications = () => {
    notificationService.showInfo('Информация', 'Это информационное уведомление');
    notificationService.showError('Ошибка', 'Произошла ошибка при выполнении операции');
    notificationService.showUpdate('Обновление', 'Доступно новое обновление приложения');
    notificationService.showReminder('Напоминание', 'Не забудьте заполнить профиль');
    notificationService.showPromo('Акция', 'Специальное предложение для вас', undefined, new Date(Date.now() + 86400000)); // истекает через 24 часа
  
    // Добавляем дополнительные уведомления для демонстрации фильтрации
    notificationService.showSystemNotification('Системное', 'Тестовое системное уведомление', NotificationSubtype.INFO);
    notificationService.showUserNotification('Пользовательское', 'Тестовое пользовательское уведомление', NotificationSubtype.INFO);
  };
