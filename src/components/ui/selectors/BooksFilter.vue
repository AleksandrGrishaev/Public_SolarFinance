...
// Определение режима выбора - ВАЖНО: используем только props.multiSelect
...
const handleToggleBook = (bookId) => {
  if (props.debug) {
    ...
    console.log(`[BooksFilter] MultiSelect mode:`, isMultiSelectMode.value);
  }
...
const toggleBook = (bookId) => {
  ...
  const index = currentSelection.indexOf(bookId);
  
  let newSelection;
  if (index === -1) {
    // Добавляем книгу
    newSelection = [...currentSelection, bookId];
  } else if (currentSelection.length > 1) {
    // Удаляем книгу, если выбрано больше одной
    newSelection = [...currentSelection];
    newSelection.splice(index, 1);
  } else {
    // Нельзя удалить последнюю выбранную книгу
    newSelection = currentSelection;
  }
...
watch([filteredBookOptions, () => props.modelValue], ([books, value]) => {
  ...
  if (Array.isArray(value)) {
    ...
    if (!hasValidBook && books.length > 0) {
      emit('update:modelValue', isMultiSelectMode.value ? [books[0].id] : books[0].id);
    }
